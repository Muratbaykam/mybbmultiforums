...
// HERE WE HANDLE OUR FORUM ADS AS WELL AS OUR FORUM ACTIVATION...

$fname = preg_replace("/[^a-zA-Z0-9s]/", "", $ref); // Get the forum name...
...
if(file_exists("../inc/admanager.php")){

	require_once("../inc/admanager.php");

	$hads = showads("forumheader", $fname);
	$fads = showads("forumfooter", $fname);

}

// FORUM ACTIVATION CODE HERE...
...
$actmessage = "<center><b><a href='http://www.".$domain."/activate.php?forum=".$fname."'>This forum has not yet been activated!</a><br><a href='http://www.".$domain."/activate.php?forum=".$fname."'>Click Here to activate and remove this message.</a></b><br><br></center>";

$result = $db->query("SELECT * FROM ".$dbname.".site_hosted_forums WHERE actstatus < 1 AND forumname='".$fname."'");
...
// Attempt to rebuild the forum stats...

include("inc/functions_rebuild.php");

rebuild_stats();

}

// END FORUM ACTIVATION CODE HACK
...
// Ugly hack to get forum ads working

if ($flag == 1){
	eval("\$header = \"".$templates->get("header")."".$actmessage."\";");
}
else {
	eval("\$header = \"".$templates->get("header")."".$hads."\";");
}
