...
if($mybb->input['action'] == "editgroup")
{
	...
	if($mybb->request_method == "post")
	{
		...
		// Prevent the user from editing the server settings
		/*if($mybb->input['gid'] == 17){
		
			$errors[] = "You cannot edit the server settings for your forum.  For assistance please see your forum host.";
...
if($mybb->input['action'] == "change")
{
	...
	if($mybb->request_method == "post")
	{
		...
		// MYBB MULTIFORUMS - Prevent user from updating certain settings...
		
		$failureCheck = false;
		
		if(isset($mybb->input['upsetting']['load'])){
			if($mybb->input['upsetting']['load'] != 0){
				$failureCheck = true;
			}
		}
		
		if(isset($mybb->input['upsetting']['avataruploadpath'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['uploadspath'])){
				$failureCheck = true;
		}
		...
		if(isset($mybb->input['upsetting']['errorloglocation'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['maxmessagelength'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['mail_handler'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['mail_parameters'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['smtp_host'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['smtp_user'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['smtp_pass'])){
				$failureCheck = true;
		}
		
		if(isset($mybb->input['upsetting']['smtp_port'])){
				$failureCheck = true;
		}
		...
		admin_redirect("index.php?module=config-settings");
			...
			flash_message("One or more of the settings you are trying to update is disabled by your forum host.  None of your changes have been saved!", 'error');
			admin_redirect("index.php?module=config-settings");
	...
	foreach($cache_groups as $groupinfo)
	{
		...
		foreach($cache_settings[$groupinfo['gid']] as $setting)
		{
			...
			// Prevent certain settings from showing in group listing
			if($setting['name'] == "avataruploadpath"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "uploadspath"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "load"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "errorloglocation"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "maxmessagelength"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "mail_handler"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "mail_parameters"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "smtp_host"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "smtp_port"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "smtp_user"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "smtp_pass"){
				$failureCheck = true;
			}
			
			if($setting['name'] == "secure_smtp"){
				$failureCheck = true;
			}
